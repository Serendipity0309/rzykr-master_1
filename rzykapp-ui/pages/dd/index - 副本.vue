<template>
	...
	<view>
		<view v-if="item.length<1">
			<image class="bg-img" src="../../static/notice.png"></image>
		</view>
		<view v-for="elm in item">
			<uni-card :isShadow="true" :title="elm.processName" mode="basic" thumbnail="/static/favicon.png"
				note="true">
				<view>
					<view>:<text v-text="elm.name"></text></view>
				...
				<view>
					<view class="footer-box">
						...
						<view @click="finishIndividualTask(elm.examData.id,1)" v-if="permission==2">
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				const Authorization = `Bearer ${uni.getStorageSync("token")}`;
				uni.request({
					url: baseUrl + "/act/QueryProcess?query=" + 2 + "&id=" + id,
					header: {
						Authorization
					},
					method: 'GET',
